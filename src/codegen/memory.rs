...
pub fn declare_alloc_fns(module: &mut ObjectModule) {
    ...
    module
        .declare_function("__kiln_rc_inc", Linkage::Import, &rc_inc_sig)
        .ok();

    let mut rc_dec_sig = module.make_signature();
    rc_dec_sig.params.push(AbiParam::new(types::I64));
    module
        .declare_function("__kiln_rc_dec", Linkage::Import, &rc_dec_sig)
        .ok();
}
...
pub fn emit_malloc(
    ...
    let callee = module
        .declare_function("malloc", Linkage::Import, &{
...
pub fn emit_free(ptr: Value, module: &mut ObjectModule, builder: &mut FunctionBuilder) {
    let callee = module
        .declare_function("free", Linkage::Import, &{
            let mut s = module.make_signature();
            s.params.push(AbiParam::new(types::I64));
            s
        })
